...
static chd_file* OpenCHD(const std::string& filename, FileSystem::ManagedCFilePtr fp, Error* error, u32 recursion_level)
{
	...
	for (const auto& [filename, header] : s_chd_hash_cache)
	{
		if (!StringUtil::compareNoCase(parent_dir, Path::GetDirectory(filename)))
			continue;

		if (!IsHeaderParentCHD(header, header))
			continue;

		// Re-check the header, it might have changed since we last opened.
		chd_header parent_header;
		auto parent_fp = FileSystem::OpenManagedSharedCFile(filename.c_str(), "rb", FileSystem::FileShareMode::DenyWrite);
			...
			IsHeaderParentCHD(header, parent_header))
		{
			...
			const std::string filename_to_open = filename;
