...
bool DInputSource::Initialize(SettingsInterface& si, std::unique_lock<std::mutex>& settings_lock)
{
	...
	PFNDIRECTINPUT8CREATE create = reinterpret_cast<PFNDIRECTINPUT8CREATE>(reinterpret_cast<void*>((GetProcAddress(m_dinput_module, "DirectInput8Create"))));
	if (!create)
	{
...
void DInputSource::Shutdown()
{
	while (!m_controllers.empty())
	{
		...
		InputManager::OnInputDeviceDisconnected(InputBindingKey{{.source_type = InputSourceType::DInput, .source_index = index}},
