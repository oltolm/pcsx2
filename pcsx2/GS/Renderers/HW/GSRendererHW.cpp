	...
	: GSRenderer()
{
	...
	memset((void*)&m_conf, 0, sizeof(m_conf));
...
void GSRendererHW::ResetStates()
{
	...
	memset((void*)&m_conf, 0, reinterpret_cast<const char*>(&m_conf.cb_vs) - reinterpret_cast<const char*>(&m_conf));
}
...
{
	for (const auto& draw_transfer : m_draw_transfers)
	{
	...
	GSTexture* rt, GSTexture* ds, float rt_scale, GSTexture* tex, float tex_scale, const GSVector4i& unscaled_rect)
{
	...
	std::memset((void*)&config.cb_vs, 0, sizeof(config.cb_vs));
	std::memset((void*)&config.cb_ps, 0, sizeof(config.cb_ps));
