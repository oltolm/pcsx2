	...
	DXGI_FORMAT uav_format)
{
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> resource_fbl;
	...
	if (type == Type::RenderTarget)
	{
		...
		HRESULT hr = dev->GetAllocator()->AllocateMemory(&allocationDesc, &allocInfo, &allocation);
		if (FAILED(hr))
		{
			// OOM isn't fatal.
			if (hr != E_OUTOFMEMORY)
